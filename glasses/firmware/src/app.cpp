...
void loop_app()
{
    ...
    if (photoDataUploading && fb) {
        ...
        if (remaining > 0) {
            ...
            // Verify connection before notify
            if (!connected) {
                Serial.println("[ERROR] BLE disconnected during upload. Aborting.");
            ...
            // Increased delay to allow BLE buffer to clear and prevent notify failures
            // The rc=-1 error indicates the notification queue is full
            delay(BLE_PHOTO_TRANSFER_DELAY * 3); // 9ms instead of 3ms
        } else {
            // End of photo marker
    ...
    if (audioDataUploading && audioBuffer && audioBufferSize > 0) {
        ...
        if (remaining > 0) {
            ...
            // Verify connection before notify
            if (!connected) {
                Serial.println("[ERROR] BLE disconnected during audio upload. Aborting.");
